...
int main(int argc, char *argv[])
{
    ...
    if(openDB(QStringLiteral("libdb"), true, false))
    {
        ...
        if(version != 7){
            splash->hide();
            if(QMessageBox::question(nullptr,QApplication::translate("MainWindow", "Database"), QApplication::translate("MainWindow", "This version needs new database version. All your old books data will be lost. Continue?"),QMessageBox::Yes|QMessageBox::No,QMessageBox::No)==QMessageBox::Yes)
            {
        ...
        if(nMinorVersion == 0){
            ...
            query.exec(QStringLiteral("INSERT OR REPLACE INTO params (id, name, value) VALUES ((SELECT id FROM params WHERE name = 'version_minor'), 'version_minor', 1)"));
            ...
            query.exec(QStringLiteral("CREATE TABLE 'book_tag' ('id_book' INTEGER NOT NULL,'id_tag'	INTEGER NOT NULL, FOREIGN KEY('id_tag') REFERENCES 'tag'('id'), FOREIGN KEY('id_book') REFERENCES 'book'('id'));"));
            query.exec(QStringLiteral("CREATE TABLE 'seria_tag' ('id_seria'	INTEGER NOT NULL,'id_tag' INTEGER NOT NULL, FOREIGN KEY('id_seria') REFERENCES 'seria'('id'), FOREIGN KEY('id_tag') REFERENCES 'tag'('id'));"));
            query.exec(QStringLiteral("CREATE TABLE 'author_tag' ('id_author' INTEGER NOT NULL, 'id_tag' INTEGER NOT NULL, FOREIGN KEY('id_tag') REFERENCES 'tag'('id'), FOREIGN KEY('id_author') REFERENCES 'author'('id'));"));
        }
    }
