	...
	// Edit libraries
		...
		// Get library information
			 ...
			 << "Library:\t" << mLibs[nId].name.toStdString() << "\n"
                         ...
                         << QApplication::translate("LibrariesDlg", "OPDS server").toStdString() << ":\thttp://localhost:" << options.nOpdsPort << "/opds_" << nId << "\n"
                         << QApplication::translate("LibrariesDlg", "HTTP server").toStdString() << ":\thttp://localhost:"  << options.nOpdsPort << "/http_" << nId << "\n";
	        ...
	        // Set library path
                if(cmdparam=="--lib-sp"){
                        ...
                        QString inpxPath = parseOption(argc-(i), &argv[i], "-ipath");
			...
			libPath=QFileInfo{libPath}.absoluteFilePath();
				...
				if ( QFile::exists(libPath)){
				   ...
				   std::cout << "The lib path " + libPath.toStdString() + " does not exists\n";
			        }
				   ...
				   std::cout << "The inpx path " + inpxPath.toStdString() + " does not exists\n";
			        }
			}
			else{
				cmdhelp();
				 ...
				 std::cout << "Name: \t" << sName.toStdString() << "\tPath: \t" << sPath.toStdString() << "\t - Ok!\n";
                            ...
                            std::cout << "\nThe lib path " + sPath.toStdString() + " does not exists\n\n";
			    cmdhelp();
		    }
                     return 0;
                    }
