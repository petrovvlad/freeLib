...
//    abc_glasnie=ts.readLine();
  ...
  //      file.close();
    ...
    foreach (QString str, pattern)
    {
     ...
     qSort(plist.list.begin(),plist.list.end(),pattern_compare);
//     qDebug()<<exceptions.count()<<pattern.count();
}
...
QString hyphenations::hyphenate_word(QString word_original, QString separator,bool hyphenation_only)
{
    ...
    foreach (QString word, words)
    {
        ...
        {
            result_word+=((last_word_empty || result_word>0)?"-":"")+word;
    ...
    //        return word;
        ...
        QString word_string=QString("."+word+".").toLower();
        QVector<int> levels;
        ...
        for(int i=0;i<word_string.length()-2;++i)
        {
            ...
            for(int count=1;count<=word_string.length()-i;++count)
            {
                ...
                if (pattern_compare(&pattern_from_word, *pattern_iter))
                        continue;
                pattern_iter = qLowerBound(pattern_iter, plist.list.end(), &pattern_from_word, pattern_compare);
                if (pattern_iter == plist.list.end())
                        break;
                if (!pattern_compare(&pattern_from_word, *pattern_iter))
                {
      //              qDebug()<<pattern_from_word.str<<((pattern_t*)*pattern_iter)->str<<((pattern_t*)*pattern_iter)->levels;
        ...
        for(int i=0;i<wh.count();++i)
        {
            if(levels[i+1]%2 && i)
                ...
                wh[i]=0;
           // qDebug()<<levels[i+1]<<levels[i+1]%2<<bool(wh[i]);
        }
        QString result;
        int len=word.length();
